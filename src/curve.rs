...
impl ExtendedPoint {
    ...
    pub fn from_hash<D>(nonce: &[u8]) -> ExtendedPoint
        where D: Digest<OutputSize = U64> + Default {
            ...
            // Hash the nonce and map it to a point on the curve and get its
            // uniform representative:
            let (p, sign) = ExtendedPoint::from_hash_nonuniform::<D>(&nonce);
            let u: elligator::UniformRepresentative = elligator::elligator2(&p);

            // Convert to an extended twisted Edwards point and multiply to
            // kill the curve cofactor:
            q = CompressedMontgomeryX::from_x(u, sign).to_edwards_y().decompress();
            q.mult_by_cofactor();
            q
    }
    ...
    /// # Return
    ///
    /// A non-uniformly chosen `ExtendedPoint` and a mask which can optionally be
    /// used to determine the sign of the of compressed Edwards `Y` coordinate.
    fn from_hash_nonuniform<D>(nonce: &[u8]) -> (ExtendedPoint, u8)
        where D: Digest<OutputSize = U64> + Default {
            ...
            // Edwards sign bit:
            let sign = (digest[31] & 0x80) >> 7;
            digest[31] &= 0x7f;

            // Map the hash to a point on the curve and get its uniform representative:
            h = Scalar::reduce(&digest);
            p = ExtendedPoint::basepoint_mult(&h);
