...
#[cfg(feature = "elligator")]
use elligator::UniformRepresentative;
...
impl CompressedEdwardsY {
    ...
    pub fn to_montgomery_x(&self) -> CompressedMontgomeryU {
...
impl ExtendedPoint {
    ...
    pub fn to_uniform_representative(&self) -> Option<UniformRepresentative> {
        UniformRepresentative::encode(&self)
    }
...
#[cfg(feature = "elligator")]
impl ExtendedPoint {
    /// Hash to a uniformly-random point in a prime-order subgroup of the curve.
    ///
    /// The provided nonce is hashed with keccak-512.  The high-bit of the
    /// produced hash digest is used to determine the eventual sign bit of the
    /// extended twisted Edwards point. The rest of the digest is reduced to a
    /// `Scalar` in ℤ/lℤ, and multiplied by the basepoint to produce a point on
    /// the curve. We use the Elligator2 mapping to produce a uniform
    /// representative of the point, then convert to extended twisted Edwards
    /// coordinates and multiply by the curve cofactor.
    ///
    /// # Examples
    ///
    /// This should be called on any hash function which gives 512-bits of output.
    /// (Suitable examples are Sha2-512, Keccak-512, Blake2b-512, etc.)
    ///
    /// ```
    /// use rand::OsRng;
    /// use sha3::Keccak512;
    ...
    /// # Return
    ///
    /// A uniformly random point, in a prime-order subgroup of the curve, namely,
    /// the order-q subgroup generated by the basepoint.
    //
    // XXX Do we want to do the final multiplication by the cofactor, or tell
    //     people to use decaf for that?
    pub fn from_hash<D>(nonce: &[u8]) -> ExtendedPoint
        where D: Digest<OutputSize = U64> + Default {
            ...
            // Use the hash function to produce a digest of the nonce:
            hasher.input(&nonce[..]);
            digest.copy_from_slice(hasher.result().as_slice());

            // Save the high bit of the digest for later use as the compressed
            // Edwards sign bit:
            s = (digest[31] & 0x80) >> 7;
            digest[31] &= 0x7f;
            ...
            let y:          FieldElement  = CompressedMontgomeryU::to_edwards_y(&u);
            let x:          FieldElement  = CompressedMontgomeryU::to_edwards_x(&u, &v, &s);

            // Convert to an extended twisted Edwards point by {x:y:1:x*y} and
            // multiply to curve cofactor to end up in coset 8:
            q = ExtendedPoint{ X: x, Y: y, Z: FieldElement::one(), T: &x * &y };
            q.mult_by_cofactor();
            q
    }
}
...
mod test {
    ...
    // -------------------------------------------------------------------------
    // Testvectors produced with Signal code
    // -------------------------------------------------------------------------

    // hash_to_point("tsunami")
    static SIGNAL_HASH_TO_POINT_1: ExtendedPoint = ExtendedPoint{
        X: FieldElement([ 30884285, -7881781,  22627093, -14380283, -25459363,
                         -14317826,   407507,   8742521,  14136636,  -8050438, ]),
        Y: FieldElement([  6948990, 14625121,   5141165,  -5141439,   9050575,
                         -12001523, -4201299, -11643840, -23236450,   4663800, ]),
        Z: FieldElement([ 32687269, -11824231, -31621521, -3136071, -14678321, 8867975, 22100788, 4077653, 31663669, -7447383]),
        T: FieldElement([19356145, 14247399, 24121491, 1921064, 11577412, 9076885, -10450408, 14049798, -15463173, -11775035])};

    // hash_to_point("lumpy space rhinoceros")
    static SIGNAL_HASH_TO_POINT_2: ExtendedPoint = ExtendedPoint{
        X: FieldElement([-2382867, -3837344, -28951336, 4502124, 18184866, 6017643, 19495146, -13636049, 22218144, 15266964]),
        Y: FieldElement([-17061117, -12840609, -16974436, -12179333, -20453193, -11458358, 29630533, -15199539, -19344381, 3102025]),
        Z: FieldElement([18168647, -13895333, -23170982, 7124984, -15981985, 9637121, -26363828, 9076893, -16793334, 10444272]),
        T: FieldElement([28418505, -9277641, 18211900, 6559248, 29099218, 3134209, 4487748, -7671411, -8984123, -14913576])};

    // hash_to_point("zen albatross")
    static SIGNAL_HASH_TO_POINT_3: ExtendedPoint = ExtendedPoint{
        X: FieldElement([5020716, 9323579, -16173650, -2319736, -20247085, 5885898, -20977798, 12625311, -18999466, -982799]),
        Y: FieldElement([30983903, 7730106, -30354128, 9523788, 15491350, -11432086, -26798357, -4636460, -2728496, -10037143]),
        Z: FieldElement([-31652050, 7006612, -4878933, 16544806, 16511254, 11923819, -11010592, -7994329, 23926952, -13882679]),
        T: FieldElement([21138523, -10402564, 12659372, -15891942, -23301450, -2030346, -8293443, -1042521, 13814183, 13487042])};

    // hash_to_point("i'm just hoping the fire doesn't spread")
    static SIGNAL_HASH_TO_POINT_4: ExtendedPoint = ExtendedPoint{
        X: FieldElement([22870397, 2173784, -15728933, -14233040, -3231972, 12483740, 20895318, -8531961, -1734557, -6841314]),
        Y: FieldElement([27355521, 15830867, 3394603, -15033387, 533635, 9889795, 21051524, -14522148, -22167491, 589879]),
        Z: FieldElement([15290640, -5229499, -15080718, -7595974, 17130512, -13139982, 13920602, 761665, 13970855, 11945478]),
        T: FieldElement([-14603400, -6018891, -10770084, -14393102, 21075807, -16420205, -30032788, -7718728, -24295491, 10514755])};

    // -------------------------------------------------------------------------
    // Tests for ensuring we match various Signal testvectors
    // -------------------------------------------------------------------------

    #[cfg(feature = "elligator")]
    #[test]
    fn hash_to_point_matches_signal() {
        let p1: ExtendedPoint = ExtendedPoint::from_hash::<Sha512>("tsunami".as_bytes());
        let p2: ExtendedPoint = ExtendedPoint::from_hash::<Sha512>("lumpy space rhinoceros".as_bytes());
        let p3: ExtendedPoint = ExtendedPoint::from_hash::<Sha512>("zen albatross".as_bytes());
        let p4: ExtendedPoint = ExtendedPoint::from_hash::<Sha512>("i'm just hoping the fire doesn't spread".as_bytes());

        assert_eq!(p1.compress_edwards(), SIGNAL_HASH_TO_POINT_1.compress_edwards());
        assert_eq!(p2.compress_edwards(), SIGNAL_HASH_TO_POINT_2.compress_edwards());
        assert_eq!(p3.compress_edwards(), SIGNAL_HASH_TO_POINT_3.compress_edwards());
        assert_eq!(p4.compress_edwards(), SIGNAL_HASH_TO_POINT_4.compress_edwards());
    }
    ...
    #[cfg(feature = "elligator")]
    #[bench]
    fn hash_to_point(b: &mut Bencher) {
        let mut rng: OsRng = OsRng::new().unwrap();
        let mut nonce: [u8; 16] = [0u8; 16];

        rng.fill_bytes(&mut nonce);

        b.iter(| | ExtendedPoint::from_hash::<Sha512>(&nonce) )
    }
}
